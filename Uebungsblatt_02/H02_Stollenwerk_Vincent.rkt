...
(define (protanopia-map img)
  ;; Type: color -> color
  ;; Returns: Given color after simulating protanopia
  (color-list->bitmap (map (lambda (pixel-color)
         (vector->color (rgb-vector-protanopia (color->vector pixel-color))))
       (image->color-list img)) (image-width img) (image-height img)))
